...
const rules_settings = {
    ...
    MAX_LENGTH: {
        renderMessage: (max = 15) =>
            `Ce champ doit contenir un maximum de ${max} caractères`,
        validationMethod: (value, max = 15) => value.trim().length <= max,
        renderBadge: (max = 5) => `Longueur max : ${max}`,
    },
    MIN: {
        renderMessage: (min = 5) =>
            `Ce champ ne doit pas contenir de valeur plus petite que ${min}.`,
        validationMethod: (value, min = 5) => value.trim().length >= min,
        renderBadge: (min = 5) => `Min : ${min}`,
    },
    MAX: {
        renderMessage: (max = 15) =>
            `Ce champ ne doit pas contenir une valeur plus grande que ${max}.`,
        validationMethod: (value, max = 15) => value.trim().length <= max,
    ...
    TYPE_EMAIL: {
        ...
        validationMethod: (value) =>
            /^[\w+\-.]+@([\w-]+\.)+[\w-]{2,20}$/.test(value), //    longuest domain found is 14 long : .cancerresearch
        renderBadge: () => `Format courriel`,
    },
    TYPE_ALPHANUMERIC: {
        renderMessage: () =>
            "Ce champ ne doit contenir que des caractères alphanumériques (lettres et chiffres)",
        validationMethod: (value) => /^[A-Za-z0-9]*$/.test(value),
        renderBadge: () => `Alphanumerique (lettres et chiffres)`,
    },
    FILE_MAX_SIZE: {
        renderMessage: (mo = 5) =>
            `Ce champ n'accepte que les fichiers de ${mo} Mo et moins.`,
        validationMethod: (value, mo = 5) =>
            value ? value?.size <= mo * 1024 * 1024 : true,
        renderBadge: (mo = 5) => `${mo} Mo max`,
    },
    ONE_OF_MANY_REQUIRED: {
        renderMessage: () =>
            `Vous devez remplir au moins l'un des champs de cette section`,
        validationMethod: (value, dependencies) =>
            isValuePositive(value) ||
            dependencies.some((value) => isValuePositive(value)),
        renderBadge: () => `1 champ requis`,
    },
    HIGHER_DATE_REQUIRED: {
        renderMessage: () =>
            `La date dans ce champ doit être ultérieure à celle du prédécent`,
        validationMethod: (value, dependencies) =>
            !dependencies.some(
                (dep) => new Date(dep).getTime() > new Date(value).getTime()
            ),
...
const initiateValidator = (selectedRules) => {
    ...
    for (const settingRuleName in rules_settings) {
        ...
        const matchingValidator = selectedRules.find(
            (rule) => rule.name === settingRuleName
        );
        ...
        if (matchingValidator)
            ...
            newFieldValidators[settingRuleName] = {
                ...
                message: rules_settings[settingRuleName].renderMessage(
                    matchingValidator.specification &&
                        matchingValidator.specification
                ),
                validationMethod:
                    rules_settings[settingRuleName].validationMethod,
                badge: rules_settings[settingRuleName].renderBadge(
                    matchingValidator.specification &&
                        matchingValidator.specification
                ),
                isValid: false,
...
export const useValidation = (setOfRules, formState) => {
    ...
    let ONE_OF_MANY_DEPENDECIES =
        formState && validator["ONE_OF_MANY_REQUIRED"]
            ? validator["ONE_OF_MANY_REQUIRED"].dependencies.map((dep) =>
                  dep.listenerValue(formState)
              )
            : [];
    let HIGHER_DATE_REQUIRED =
        formState && validator["HIGHER_DATE_REQUIRED"]
            ? validator["HIGHER_DATE_REQUIRED"].dependencies.map((dep) =>
                  dep.listenerValue(formState)
              )
            : [];
    ...
    const validate = (value) => {
        ...
        for (const ruleName in validator) {
            ...
            if (!rule.specification && rule.dependencies)
                ...
                param = rule.dependencies.map((depFunction) =>
                    depFunction.value(formState)
                );
    ...
    const RequirementsBadges = (props) => {
        ...
        const forceBadgeDisplay = Object.values(validator)
            .map((rule) => rule.isValid)
            ...
            <>
                ...
                {rulesNameList.length > 0 && (
                    <div className="w-100">
                        ...
                        <ul
                            className={`mb-0 ${addUlPadding && "form-element--field-padding-top-reverse"} ${alwaysDisplay && "d-flex"} badge-container gap-2 ${forceBadgeDisplay ? "form-element--force-badge-display" : ""}`}
                        >
    ...
    const ValidationErrorMessages = () => {
        ...
        const errorRulesList = completeRulesList.filter(
            (ruleName) => validator[ruleName].isValid === false
        );
        ...
        return (
            <>
                ...
                {errorRulesList.length > 0 && (
                    <ul
                        className={`mt-1 mb-0 d-inline-flex flex-wrap form-element--validation-errors-container `}
                    >
