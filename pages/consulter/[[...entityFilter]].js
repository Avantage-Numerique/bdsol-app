...
const ConsultData = (props) => {
  ...
  const currentQueryEntityFilterUrl = () => {
    return Array.isArray(props.entityFilters) && props.entityFilters.length > 0
  ...
  async function sendApiListRequest(directSkipNumber = null) {
    ...
    const res = await searchByType(ORIGIN_BROWSER, consultData.entities[0], {
      skip: targetSkip,
    });
