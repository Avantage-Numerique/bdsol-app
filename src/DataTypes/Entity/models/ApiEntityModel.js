...
class ApiEntityModel {
    ...
    static domainsToSelectOptions(domains) {
        ...
        if (domains?.length === undefined && domains?.domain)
            return [
                {
                    value: domains.domain._id,
                    label: domains.domain.name,
                    color: getColor(domains.domain),
                },
            ];
    ...
    static locationToSelectOptions(entity) {
        if (entity.address)
            return [
                {
                    value: entity._id,
                    label: entity.address + ", " + entity.name,
                },
            ];
        else return [{ value: entity._id, label: entity.name }];
    }
}
