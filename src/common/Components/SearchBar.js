...
const SearchBar = ({ small, className, ...props }) => {
    ...
    const getNearestTaxonomyToSearchIndex = async (searchIndex) => {
        const nearestTaxonomyResponse = await clientSideExternalApiRequest(
            "/search/nearestTaxonomy?searchIndex=" + formState.inputs.searchIndex.value,
            { method: "GET" }
        );
    ...
    const formRequestData = (val) => {
        setInputValue(val);
        inputHandler("searchIndex", val, props.validators ? validate(event.target.value, props.validators) : true);
    };
        ...
        <form onSubmit={submitHandler} className={`search-bar ${className} ${small && "small-searchBar w-100"}`}>
            <div className="input-group my-2 bg-white">
                <Select
                    ...
                    onChange={(val, action) => {
                        if (action.action === "select-option" && !blockSubmitSlug.current) {
                    ...
                    noOptionsMessage={(val) => (
                        <p className={"m-0 p-0"}>
                            ...
                            Appuyez sur la touche <Icon iconName={"keyboard"} /> <code>Entrer</code> pour rechercher
                            avec cette valeur quand même.
                        </p>
                    )}
                    components={{
