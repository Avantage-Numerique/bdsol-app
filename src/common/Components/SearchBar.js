...
const SearchBar = ({ small, className, ...props }) => {
    ...
    const { FormUI, submitRequest, formState, formTools, clearFormData } =
        useFormUtils({
            searchIndex: {
    ...
    useEffect(() => {
        ...
        if (selectResponse?.data?.length > 0)
            resultsOptions = selectResponse.data.map((elem) => {
                return {
                    value: elem._id,
                    label: elem.name ?? elem.firstName + " " + elem.lastName,
                    type: elem.type,
                    slug: elem.slug,
        ...
        //remove duplicate
        const uniqueOptions = [...resultsOptions, ...nearTaxoOptions].filter(
    ...
    const getNearestTaxonomyToSearchIndex = async (searchIndex) => {
        const nearestTaxonomyResponse = await clientSideExternalApiRequest(
            "/search/nearestTaxonomy?searchIndex=" +
                formState.inputs.searchIndex.value,
            { method: "GET" }
        );
        let nearTaxoToEntityArray = [];
        if (nearestTaxonomyResponse?.data?.nearestTaxonomy)
            nearTaxoToEntityArray.push(
                nearestTaxonomyResponse.data.nearestTaxonomy
            );

        if (nearestTaxonomyResponse?.data?.linkedEntityToNearestTaxonomy)
            nearTaxoToEntityArray.push(
                ...nearestTaxonomyResponse.data.linkedEntityToNearestTaxonomy
            );
    ...
    const formRequestData = (val) => {
        ...
        inputHandler(
            ...
            val,
            props.validators
                ? validate(event.target.value, props.validators)
                : true
        );
    };
    ...
    return (
        <form
            ...
            className={`search-bar ${className} ${small && "small-searchBar w-100"}`}
        >
            <div className="input-group my-2 bg-white">
                <Select
                    ...
                    placeholder={
                        "Rechercher une personne, un projet, un équipement, etc."
                    }
                    value={value}
                    options={optionList}
                    ...
                    onKeyDown={(event) => {
                        if (event.key === "Enter") {
                            blockSubmitSlug.current = true;
                            submitHandler();
                        }
                    }}
                    noOptionsMessage={(val) => (
                        <p className={"m-0 p-0"}>
                            Aucune suggestion trouvé avec la recherche{" "}
                            <strong>{val.inputValue}</strong>.<br />
                            Appuyez sur la touche <Icon
                                iconName={"keyboard"}
                            />{" "}
                            <code>Entrer</code> pour rechercher avec cette
                            valeur quand même.
                        </p>
                    )}
                    components={{
