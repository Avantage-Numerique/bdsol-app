...
const rules_settings = {
    ...
    ONE_OF_MANY_REQUIRED: {
        ...
        validationMethod: (value, dependencies) =>
            isValuePositive(value) || dependencies.some((value) => isValuePositive(value)),
    ...
    HIGHER_DATE_REQUIRED: {
        ...
        validationMethod: (value, dependencies) =>
            !dependencies.some((dep) => new Date(dep).getTime() > new Date(value).getTime()),
        ...
        if (matchingValidator)
            ...
            newFieldValidators[settingRuleName] = {
                ...
                message: rules_settings[settingRuleName].renderMessage(
                    matchingValidator.specification && matchingValidator.specification
                ),
...
export const useValidation = (setOfRules, formState) => {
    ...
    let ONE_OF_MANY_DEPENDECIES =
        formState && validator["ONE_OF_MANY_REQUIRED"]
            ? validator["ONE_OF_MANY_REQUIRED"].dependencies.map((dep) => dep.listenerValue(formState))
            : [];
    let HIGHER_DATE_REQUIRED =
        formState && validator["HIGHER_DATE_REQUIRED"]
            ? validator["HIGHER_DATE_REQUIRED"].dependencies.map((dep) => dep.listenerValue(formState))
            : [];
    ...
    const RequirementsBadges = (props) => {
        const { addUlPadding, alwaysDisplay, displayOnlyBadges = false } = props;
