...
const Spinner = ({ fixed, absolute, reverse, className, label, loadingState }) => {
    ...
    return (
        <div className={`${styles["spinner__container"]}  ${className} ${fixed ? styles["spinner__container--fixed"] : ""} ${absolute ? styles["spinner__container--absolute"] : ""} ${currentAnimateClass}`}>
