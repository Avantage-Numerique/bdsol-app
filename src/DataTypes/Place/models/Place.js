...
class Place extends EntityModel {
    ...
    getBreadCrumbTitle(raw) {
        if (raw.address) {
            if (raw.name) return raw.address + ", " + raw.name;
            if (raw.city) return raw.address + ", " + raw.city;
            if (raw.postalCode) return raw.address + ", " + raw.postalCode;
            return raw.address; //We probably don't want to only show address if there is no city or no postalcode
        }
        if (raw.name) {
            if (raw.city) return raw.name + ", " + raw.city;
            if (raw.postalCode) return raw.name + ", " + raw.postalCode;
