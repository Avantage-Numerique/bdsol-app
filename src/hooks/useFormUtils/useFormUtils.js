...
export const useFormUtils = (initialState, actions) => {
    ...
    const submitRequest = async (
        ...
        params = { isBodyJson: true }
    ) => {
        let response;
        if (formState.isValid) {
            //Send the request with the specialized hook
            response = await sendRequest(route, type, data, header, params);
            ...
            if (!response.error) {
                ...
                if (innerMessage) setInnerMessage("");

                /* Actions executed when the form is positive */

                //1. Clear the form
                if (positiveResponseActions.clearForm) clearFormData();
            ...
            //Inform the user
            setInnerMessage(
                "Attention. Le formulaire envoyé n'est pas valide. Assurez-vous que tous les champs sont bien remplis."
            );
        }
        return response;
    };
